...
const ProductsMenu = ({ dropName }) => {
    ...
    const uniqueItems = productData.filter((data, index, self) => 
        self.findIndex(item => (dropName === "Categories" ? item.category === data.category : item.subcategory === data.subcategory)) === index
    );

    const catHandler = (key) => {
        setAnchorEl(null);
        dispatch(getSearchedProducts(dropName === "Categories" ? "searchProductbyCategory" : "searchProductbySubCategory", key));
        if (location.pathname !== "/ProductSearch") navigate("/ProductSearch");
    };
        ...
        <div style={{ marginLeft: "2rem" }}>
            ...
            >
                {uniqueItems.map((data) => (
                    <MenuItem onClick={() => catHandler(dropName === "Categories" ? data.category : data.subcategory)} key={data._id}>
