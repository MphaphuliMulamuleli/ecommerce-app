...
const OrderSummary = ({ handleNext, handleBack }) => {
    ...
    const totalOGPrice = cartDetails.reduce((total, item) => total + (item.quantity * item.price.Retail_Price), 0);
                    ...
                    <List disablePadding>
                        <ListItem sx={{ py: 1, px: 0 }}>
                            ...
                            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
                                {`R${productDetailsCart.price && productDetailsCart.price.Retail_Price * productDetailsCart.quantity}`}
                            </Typography>
                        </ListItem>
                        <ListItem sx={{ py: 1, px: 0 }}>
                            <ListItemText primary="Discount" />
                            <Typography variant="subtitle1" sx={{ color: "green" }}>
                                R{productDetailsCart.price && productDetailsCart.price.Retail_Price - productDetailsCart.price.cost}
                            </Typography>
                        </ListItem>
                        <ListItem sx={{ py: 1, px: 0 }}>
                        ...
                        <ListItem sx={{ py: 1, px: 0 }}>
                            ...
                            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
                                R{productDetailsCart.price && productDetailsCart.price.cost * productDetailsCart.quantity}
                            </Typography>
                        </ListItem>
                    </List>
                    ...
                    <List disablePadding>
                        {cartDetails.map((product, index) => (
                            <ListItem key={index} sx={{ py: 1, px: 0 }}>
                                ...
                                <Typography variant="body2">{`R${product.quantity * product.price.Retail_Price}`}</Typography>
                            </ListItem>
                        ))}
                        <ListItem sx={{ py: 1, px: 0 }}>
