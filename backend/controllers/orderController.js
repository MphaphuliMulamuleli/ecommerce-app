...
const getOrderedProductsByAdmin = async (req, res) => {
    try {
        const AdminId = req.params.id;

        const ordersWithAdminId = await Order.find({
            'orderedProducts.Admin': AdminId
        });

        if (ordersWithAdminId.length > 0) {
            const orderedProducts = ordersWithAdminId.reduce((accumulator, order) => {
