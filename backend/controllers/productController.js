...
const getProducts = async (req, res) => {
    try {
        let products = await Product.find().populate("Admin", "shopName");
...
const getAdminProducts = async (req, res) => {
    try {
        let products = await Product.find({ Admin: req.params.id })
        if (products.length > 0) {
            res.send(products)
        } else {
            res.send({ message: "No products found" });
...
const deleteProducts = async (req, res) => {
    try {
        const deletionResult = await Product.deleteMany({ Admin: req.params.id });
        ...
        const deletedProducts = await Product.find({ Admin: req.params.id });
...
const getAddedToCartProducts = async (req, res) => {
    try {
        ...
        const customersWithAdminProduct = await Customer.find({
            'cartDetails.Admin': AdminId
        });

        const productMap = new Map(); // Use a Map to aggregate products by ID
        customersWithAdminProduct.forEach(customer => {
            customer.cartDetails.forEach(cartItem => {
                if (cartItem.Admin.toString() === AdminId) {
        ...
        if (productsInCart.length > 0) {
            ...
            res.send({ message: 'No products from this Admin are added to cart by customers.' });
        }
    } catch (error) {
        res.status(500).json(error);
    }
